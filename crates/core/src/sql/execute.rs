        ...
        Statement::Select(stmt) => {
            ...
            let rows = execute_select_stmt(stmt, &DeltaTx::from(&*tx), &mut metrics, |plan| {
                check_row_limit(
                    ...
                    db,
                    &tx,
                    |plan, tx| plan.plans().map(|plan| estimate_rows_scanned(tx, plan)).sum(),
                    &auth,
                )?;
                Ok(plan)
            })?;
...
pub(crate) mod tests {
    ...
    fn test_rls_rules() -> ResultTest<()> {
        ...
        assert_eq!(
            run(&db, "select * from users", auth_for_id_1, None, &mut vec![])?,
            vec![product![id_1]]
        );
        assert_eq!(
            run(&db, "select * from users", auth_for_id_2, None, &mut vec![])?,
            vec![product![id_2]]
        );
        assert_eq!(
            run(&db, "select * from sales", auth_for_id_1, None, &mut vec![])?,
            vec![product![1u64, 1u64, 1u64, id_1], product![3u64, 2u64, 3u64, id_1]]
        );
        assert_eq!(
            run(&db, "select * from sales", auth_for_id_2, None, &mut vec![])?,
