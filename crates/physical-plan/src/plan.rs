...
impl ProjectListPlan {
    ...
    pub fn plans(&self) -> impl Iterator<Item = &PhysicalPlan> + '_ {
        match self {
...
mod tests {
    ...
    fn limit() {
        ...
        assert!(plan.plans().any(|plan| plan.has_filter()));
        assert!(plan.plans().any(|plan| plan.has_table_scan(None)));
    }
}
